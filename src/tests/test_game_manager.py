...
def mock_dependencies(monkeypatch):
    ...
    # Mock the chess engine so we don't need Stockfish installed to run tests
    mock_engine = MagicMock()
    # Configure the mock engine's play() method to return a specific move
...
def mock_config():
    ...
    # Fake path for stockfish since it's mocked
    return {
...
def test_play_turn_legal_move(mock_dependencies, mock_config):
    ...
    manager = GameManager(config=mock_config, db_session=db_session, llm_handler=llm_handler, persona_name="Test")
    # Set a specific board state where e7e5 is a legal move for black
    manager.board = chess.Board("rnbqkbnr/pppp1ppp/8/4p3/4P3/8/PPPP1PPP/RNBQKBNR w KQkq - 0 2")
...
def test_play_turn_illegal_move(mock_dependencies, mock_config):
    ...
    llm_handler.get_response.side_effect = [
        ("a1a2", "An illegal move."), # This move is illegal for white at the start
    ...
    from gambit.game_manager import log_move
    # Assert that the illegal move was logged
    assert log_move.call_args_list[0].kwargs['is_legal'] is False
    # Assert that the final legal move was logged
