...
class GameManager:
    def __init__(
        ...
        self.llm_color = llm_color  # <-- NEW: The color the LLM is playing
        ...
        self.game_record = database.get_or_create_game(
            self.db_session, self.persona_name
        )
    ...
    def play_game(self):
        try:
            while not self.board.is_game_over():
                # UPDATED: The logic now correctly checks whose turn it is
            ...
            print(
                f"Game {self.game_record.id} finished. Outcome: {self.game_record.final_outcome}"
            )
        finally:
            self.engine.quit()
    ...
    def _play_llm_turn(self):
        ...
        for _ in range(3):
            move_notation, commentary = self.llm_handler.get_response(
            ...
            try:
                ...
                conversation_history.append(
                    {
                        "role": "user",
                        "content": f"The move '{move_notation}' is illegal. Please provide a legal move.",
                    }
                )
